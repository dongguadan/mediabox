...
const IUINT32 IKCP_RTO_MIN = 5;		// normal min rto
...
int ikcp_recv(ikcpcb *kcp, char *buffer, int len)
{
	...
	/*
	char tmp[128];
	sprintf_s(tmp, "ikcp_recv : nrcv_que= %d", kcp->nrcv_que);
	...
	*/
	...
	//OutputDebugStringA("ikcp_recv: merging");
	// merge fragment
	...
	//sprintf_s(tmp, "ikcp_recv :merged-> nrcv_que= %d", kcp->nrcv_que);
	//OutputDebugStringA(tmp);
	...
	//sprintf_s(tmp, "ikcp_recv : moved-> nrcv_que= %d", kcp->nrcv_que);
	//OutputDebugStringA(tmp);
...
int ikcp_send(ikcpcb *kcp, const char *buffer, int len)
{
	...
	/*
	char data[128];
	sprintf_s(data, "kcp :: nsnd_que= %d, sndbuf= %d", kcp->nsnd_que, kcp->nsnd_buf);
...
int ikcp_input(ikcpcb *kcp, const char *data, long size)
{
	...
	while (1) {
		...
		{
			//printf("ikcp_input : size(%lld) < (int)IKCP_OVERHEAD(%d)\n", size, (int)IKCP_OVERHEAD);
			//std::cout << "ikcp_input error----size:" << size << " IKCP_OVERHEAD:" << (int)IKCP_OVERHEAD << std::endl;
			break;
		}
		...
		if (conv != kcp->conv)
		{
			printf("ikcp_input : conv != kcp->conv\n");
			return -1;
		}
		...
		//std::cout << "ikcp_input----size(before):" << size << " IKCP_OVERHEAD:" << (int)IKCP_OVERHEAD << std::endl;
		size -= IKCP_OVERHEAD;
		//std::cout << "ikcp_input----size(after):" << size << " IKCP_OVERHEAD:" << (int)IKCP_OVERHEAD << std::endl;
		if ((long)size < (long)len) return -2;

		if (cmd != IKCP_CMD_PUSH && cmd != IKCP_CMD_ACK &&
			cmd != IKCP_CMD_WASK && cmd != IKCP_CMD_WINS)
		{
			printf("ikcp_input : cmd error 1\n");
			return -3;
		}
		...
		ikcp_shrink_buf(kcp);
		//printf("ikcp_input-----conv:%lld, cmd:%ld, frg:%ld, wnd:%ld, ts:%ld, sn:%ld, una:%ld, len:%ld\n\n", conv, cmd, frg, wnd, ts, sn, una, len);
		if (cmd == IKCP_CMD_ACK) {
			...
			ikcp_shrink_buf(kcp);
			//fmft_log("input ack: sn=%lu rtt=%ld rto=%ld", sn, (long)_itimediff(kcp->current, ts), (long)kcp->rx_rto);
...
void ikcp_flush(ikcpcb *kcp)
{
	...
	for (i = 0; i < count; i++) {
		...
		//printf("ikcp_flush-----conv:%lld, cmd:%ld, frg:%ld, wnd:%ld, ts:%ld, sn:%ld, una:%ld, len:%ld\n\n", seg.conv, seg.cmd, seg.frg, seg.wnd, seg.ts, seg.sn, seg.una, seg.len);
	}
	...
	/*
	char tmp[128];
	sprintf_s(tmp, "ikcp: snd_wnd = %d, rmt_wn= %d \n", kcp->snd_wnd, kcp->rmt_wnd);
	OutputDebugStringA(tmp);
	*/
...
void ikcp_update(ikcpcb *kcp, IUINT32 current)
{
	...
	if (slap >= 0) {
		...
		ikcp_flush(kcp);
		//OutputDebugStringA("kcp-> flushed");
	}
}
...
int ikcp_nodelay(ikcpcb *kcp, int nodelay, int minrto, int interval, int resend, int nc)
{
	if (nodelay >= 0) {
		kcp->nodelay = nodelay;
		if (nodelay) {
			...
			if ((minrto < IKCP_RTO_MIN) || (minrto > IKCP_RTO_MAX))
			{
				kcp->rx_minrto = IKCP_RTO_MIN;
			}
			else
...
int ikcp_waitwnd(const ikcpcb *kcp)
{
	return kcp->cwnd;
}

int ikcp_waitrto(const ikcpcb *kcp)
{
	return kcp->rx_rto;
}
